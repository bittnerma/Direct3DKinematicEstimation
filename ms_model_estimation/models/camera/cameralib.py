'''
The script refers to the paper, "MeTRAbs: Metric-Scale Truncation-Robust Heatmaps for Absolute 3D Human Pose
Estimation".
script url: https://github.com/isarandi/metrabs/blob/master/src/cameralib.py
...
class Camera:
    ...
    @support_single
    def rotMat_to_camera(self, rotMat):

        rotMat = np.asarray(rotMat, np.float32)
        rotMat[:, -1:] -= self.t.T
        return self.R @ rotMat #(rotMat.T @ self.R.T).T
...
def reproject_image_aliased(
    ...
    if new_camera.distortion_coeffs is None:
        ...
        new_im_homogeneous = cv2.convertPointsToHomogeneous(newim_coords)
        ...
        '''
        oldim_coords = old_camera.camera_to_image(old_camera_coords[:, 0, :])
    else:
