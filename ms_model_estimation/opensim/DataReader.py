...
class DataReader:
    ...
    def read_opensim_model(opensimModelPath: str, getBodyInGround=False):
        ...
        # read body set
        bodySet = DataReader.read_opensim_bodySet(opensimModel)
        if getBodyInGround:
            bodyInGround = DataReader.read_opensim_ground(opensimModel)
            if bodyInGround is not None:
                bodySet.bodies.append(bodyInGround)
                bodySet.update_bodiesDict()

        # read joint set
        jointSet = DataReader.read_opnesim_jointSet(opensimModel)

        # read marker set
        markerSet = DataReader.read_opensim_markerSet(opensimModel)

        # read constraint set
        constraintSet = DataReader.read_opensim_constraint_set(opensimModelPath)
    ...
    def read_opensim_ground(opensimModel):
        ...
        for i in range(len(temp.split())):
            ...
            scale = DataReader.convert_opensim_object_to_list(scale)
    ...
    def read_opensim_bodySet(opensimModel):
        ...
        for idx in range(opensimBodySet.getSize()):
            ...
            inertia = DataReader.convert_opensim_object_to_list(inertia)

            mass = body.get_mass()
            massCenter = body.get_mass_center()
            massCenter = DataReader.convert_opensim_object_to_list(massCenter)
    ...
    def read_opnesim_jointSet(opensimModel):
        ...
        for idx in range(opensimJointSet.getSize()):
            ...
            parentOrientaion = DataReader.convert_opensim_object_to_list(parentOrientaion)
            parentLocation = opensimParentFrame.get_translation()
            parentLocation = DataReader.convert_opensim_object_to_list(parentLocation)

            opensimChildFrame = joint.get_frames(1)
            childOrientaion = opensimChildFrame.get_orientation()
            childOrientaion = DataReader.convert_opensim_object_to_list(childOrientaion)

            childLocation = opensimChildFrame.get_translation()
            childLocation = DataReader.convert_opensim_object_to_list(childLocation)

            parentPath = opensimParentFrame.getAbsolutePathString()
            parentPath = DataReader.convert_to_bodyset_path(parentPath)
            childPath = opensimChildFrame.getAbsolutePathString()
            childPath = DataReader.convert_to_bodyset_path(childPath)
    ...
    def read_opensim_markerSet(opensimModel):
        ...
        for idx in range(opensimMarkerSet.getSize()):
            ...
            location = DataReader.convert_opensim_object_to_list(location)
            name = marker.getName()
